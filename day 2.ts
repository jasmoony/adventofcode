let puzzleInput2 = `Game 1: 4 red, 5 blue, 4 green; 7 red, 8 blue, 2 green; 9 blue, 6 red; 1 green, 3 red, 7 blue; 3 green, 7 red
...
puzzleInputArray.forEach((puzzleLine) => {
  const [gameLine, roundsLine] = puzzleLine.split(":");
  let gameID = parseInt(gameLine.replace("Game ", ""));
  ...
  roundsLine
    ...
    .forEach((roundString) => {
      ...
      let cubeCombis = roundString.split(",");

      cubeCombis.forEach((cubeCombi) => {
        let [numberString, colour] = cubeCombi.trim().split(" ");
...
games.forEach((game) => {
  ...
  let powerOfSet = red * blue * green;
  answer = answer + powerOfSet;
});
...
//   game.rounds.forEach((round) => {
//     let isValidRound = round.blue <= 14 && round.red <= 12 && round.green <= 13;
//     if (!isValidRound) {
//       isValidGame = false;
//     }
//   });
