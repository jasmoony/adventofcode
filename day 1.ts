...
puzzleArray.forEach((line) => {
  let first = 0;
  let last = 0;

  var lineCopy = line;
  for (let index = 0; index < line.length; index++) {
    if (lineCopy.startsWith("one") || lineCopy.startsWith("1")) {
      first = 1;
      break;
    }
    if (lineCopy.startsWith("two") || lineCopy.startsWith("2")) {
      first = 2;
      break;
    }
    if (lineCopy.startsWith("three") || lineCopy.startsWith("3")) {
      first = 3;
      break;
    }
    if (lineCopy.startsWith("four") || lineCopy.startsWith("4")) {
      first = 4;
      break;
    }
    if (lineCopy.startsWith("five") || lineCopy.startsWith("5")) {
      first = 5;
      break;
    }
    if (lineCopy.startsWith("six") || lineCopy.startsWith("6")) {
      first = 6;
      break;
    }
    if (lineCopy.startsWith("seven") || lineCopy.startsWith("7")) {
      first = 7;
      break;
    }
    if (lineCopy.startsWith("eight") || lineCopy.startsWith("8")) {
      first = 8;
      break;
    }
    if (lineCopy.startsWith("nine") || lineCopy.startsWith("9")) {
      first = 9;
      break;
    }
  ...
  var lineCopy = line;
  for (let index = 0; index < line.length; index++) {
    if (lineCopy.endsWith("one") || lineCopy.endsWith("1")) {
      last = 1;
      break;
    }
    if (lineCopy.endsWith("two") || lineCopy.endsWith("2")) {
      last = 2;
      break;
    }
    if (lineCopy.endsWith("three") || lineCopy.endsWith("3")) {
      last = 3;
      break;
    }
    if (lineCopy.endsWith("four") || lineCopy.endsWith("4")) {
      last = 4;
      break;
    }
    if (lineCopy.endsWith("five") || lineCopy.endsWith("5")) {
      last = 5;
      break;
    }
    if (lineCopy.endsWith("six") || lineCopy.endsWith("6")) {
      last = 6;
      break;
    }
    if (lineCopy.endsWith("seven") || lineCopy.endsWith("7")) {
      last = 7;
      break;
    }
    if (lineCopy.endsWith("eight") || lineCopy.endsWith("8")) {
      last = 8;
      break;
    }
    if (lineCopy.endsWith("nine") || lineCopy.endsWith("9")) {
      last = 9;
      break;
    }
    lineCopy = lineCopy.slice(0, lineCopy.length - 1);
  }

  // let numbersInLine = line.split("").filter((character) => {
  //   let number = parseInt(character);
  //   return !isNaN(number);
  // });

  // let first = numbersInLine[0];
  // let last = numbersInLine[numbersInLine.length - 1];

  let lineSum = parseInt(first + "" + last);
  sum = sum + lineSum;
});
